...
namespace rinha_de_backend_2025_processor_availability
{
	public class Worker : BackgroundService
	{
		...
		public Worker(IDistributedCache cache, PaymentProcessorDefaultService defaultProcessor, PaymentProcessorFallbackService fallbackProcessor)
		{
			_cache = cache;
		...
		protected override async Task ExecuteAsync(CancellationToken stoppingToken)
		{
			while (!stoppingToken.IsCancellationRequested)
			{
				...
				bool IsMuchSlower(BaseResponse<HealthCheckResponse> primary, BaseResponse<HealthCheckResponse> secondary) =>
					...
					secondary.Data.Value.MinResponseTime < (primary.Data.Value.MinResponseTime / 2);
